// Public booking/enquiry microsite for a coaching center. Server component —
// no auth, indexable, fast. The "real website" Grexa advertises but doesn't ship.
// ponytail: server fetch + wa.me + UPI link. No booking-calendar until asked.
...
export default async function Microsite({ params }: { params: { businessId: string } }) {
  ...
  if (!biz) return <main style={{ padding: 40 }}><h1>Center not found</h1></main>;

  const c = biz.profile_context ?? {};
  const wa = biz.whatsapp_number ? `https://wa.me/${biz.whatsapp_number}?text=${encodeURIComponent('Hi, I want to enquire about admissions')}` : null;
  const upi = c.upiId ? `upi://pay?pa=${c.upiId}&pn=${encodeURIComponent(biz.name)}&tn=Registration` : null;
  ...
  return (
    <main style={{ maxWidth: 640, margin: '0 auto', padding: '32px 20px', lineHeight: 1.5 }}>
      ...
      {c.courses && <Section title="Courses" body={c.courses} />}
      {c.fees && <Section title="Fees" body={c.fees} />}
      {c.faculty_highlights && <Section title="Faculty" body={c.faculty_highlights} />}
      {c.usps && <Section title="Why us" body={c.usps} />}
      {c.batch_timings && <Section title="Batch timings" body={c.batch_timings} />}

      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 24 }}>
        {wa && <a href={wa} style={{ ...btn, background: '#1a7f37' }}>Enquire on WhatsApp</a>}
        {upi && <a href={upi} style={{ ...btn, background: '#1558d6' }}>Pay registration fee (UPI)</a>}
      </div>
